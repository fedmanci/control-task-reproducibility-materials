    ...
    for (n in 1:num_trials){
        ...
        if (prev_vehicle[n] == 0 && first_trial[n] == 0){
            ...
            if(veryfirst_trial[n] == 1){
                ach_g2[n,1] = pr_par[sub[n], 1];
                ach_g2[n,2] = pr_par[sub[n], 2];
            }
            else if(first_trial[n] == 1){
                ach_g2[n,1] = ach_g2[n-1,1];
                ach_g2[n,2] = ach_g2[n-1,2];
            }
            else{
                if (block_type[n] == 1){
                    ach_g2[n,1] = ach_g2[n-1,1] + int_step(reinforcement[n-1] - 45) * lr_par_w[sub[n]] * (trial_outcome[n-1] - ach_g2[n-1,1]) + int_step(-trial_outcome[n-1]) * lr_par_l[sub[n], block_type[n]] * (trial_outcome[n-1] - ach_g2[n-1,1]) ;
                    ach_g2[n,2] = ach_g2[n-1,2];
                }
                if (block_type[n] == 2){
                    ach_g2[n,2] = ach_g2[n-1,2] + int_step(-trial_outcome[n-1]) * lr_par_l[sub[n], block_type[n]] * (trial_outcome[n-1] - ach_g2[n-1,2]) ;
                    ach_g2[n,1] = ach_g2[n-1,1];
                }
            }
        }
...
model{
    ...
    for (n in 1:num_trials){
        options[1] = rew_par[sub[n], block_type[n]] * rew[n,1] + veh_par[sub[n], block_type[n]] * 100 * p1[n] - dst_par[sub[n],block_type[n]] * d11[n];# - int_step(rew[n,2] - rew[n,1]) * int_step(d11[n] - d21[n]) * 1e6;
        options[2] = rew_par[sub[n], block_type[n]] * rew[n,2] + veh_par[sub[n], block_type[n]] * 100 * p1[n] - dst_par[sub[n],block_type[n]] * d12[n] + int_step(rew[n,2] - rew[n,1]) * int_step(d22[n] - d12[n]) * 10 * mf_par[sub[n], block_type[n]] * ach_g2[n,block_type[n]];# - int_step(d12[n] - d22[n]) * 1e6;
        options[3] = rew_par[sub[n], block_type[n]] * rew[n,1] + veh_par[sub[n], block_type[n]] * 100 * p2[n] - dst_par[sub[n],block_type[n]] * d21[n];# - int_step(rew[n,2] - rew[n,1]) * int_step(d21[n] - d11[n]) * 1e6;
        options[4] = rew_par[sub[n], block_type[n]] * rew[n,2] + veh_par[sub[n], block_type[n]] * 100 * p2[n] - dst_par[sub[n],block_type[n]] * d22[n] + int_step(rew[n,2] - rew[n,1]) * int_step(d12[n] - d22[n]) * 10 * mf_par[sub[n], block_type[n]] * ach_g2[n,block_type[n]];# - int_step(d22[n] - d12[n]) * 1e6;
...
generated quantities{
    ...
    for (n in 1:num_trials){
        ...
        if (reinforcement[n] > -40){
            ...
            gen_data[n] = categorical_logit_rng(options);
        }
        else{
