...
parameters{
  ...
  vector<lower = 0, upper = 15>[2]                frequency[n_subjects];
}
...
model{
    ...
    for (s in 1:n_subjects){
        ...
        prob_par[s,1] ~     normal(hyper_m[5],hyper_v[5]);
        prob_par[s,2] ~     normal(hyper_m[6],hyper_v[6]);
    ...
    for (n in 1:n_trials){
        if (reinforcement[n] > -40){
            ...
            add_opts[1] = -d11[n] * dist_par[sub[n],block_type[n]] + g1[n] * 100 * veh_par[sub[n],block_type[n]] + prob_par[sub[n], block_type[n]] * 100 * pr[n,1] + ach_par[sub[n],block_type[n]] * 100 * ach1[n];
            add_opts[2] = -d21[n] * dist_par[sub[n],block_type[n]] + g2[n] * 100 * veh_par[sub[n],block_type[n]] + prob_par[sub[n], block_type[n]] * 100 * pr[n,2] + ach_par[sub[n],block_type[n]] * 100 * ach2[n];
