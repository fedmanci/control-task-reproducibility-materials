...
parameters{
    ...
    vector<lower = 0, upper = 1>[2]                 inter_par[n_subjects]; # interaction sensitivity

    vector<lower = 0, upper = 1>[8]                 hyper_m; # hyper means for above parameters
    vector<lower = 0, upper = 5>[8]                 hyper_v; # hyper vars for above parameters
}
...
model{
    ...
    for (s in 1:n_subjects){
        ...
        inter_par[s,1] ~    normal(hyper_m[5],hyper_v[5]);
        inter_par[s,2] ~    normal(hyper_m[6],hyper_v[6]);
        ach_par[s,1] ~    normal(hyper_m[7],hyper_v[7]);
        ach_par[s,2] ~    normal(hyper_m[8],hyper_v[8]);
    }
    ...
    for (n in 1:n_trials){
        ...
        if (reinforcement[n] > -40){
            ...
            add_opts[1] =  -d11[n]*dist_par[sub[n],block_type[n]] + ach_par[sub[n],block_type[n]]*100*ach1[n] + g1[n]*100*veh_par[sub[n],block_type[n]] - inter_par[sub[n],block_type[n]]*10*d11[n]*g1[n];
            add_opts[2] =  -d21[n]*dist_par[sub[n],block_type[n]] + ach_par[sub[n],block_type[n]]*100*ach2[n] + g2[n]*100*veh_par[sub[n],block_type[n]] - inter_par[sub[n],block_type[n]]*10*d11[n]*g2[n];
